...
outf = open('prj5d_vv1000_euler eng.txt', 'w+')
outf.write(" potential\t\tkinetic\t\ttotal\t\tangular momentum\n")
outf.close()
...
for j in range (yr):
    ...
    for i in range(N - 1):
        ...
        keteng+=keteng
        t=pot+keteng
        am=m_earth*m_sun*(vx[i+1]+vy[i+1])*r
        #am+=am
        x[i + 1] = x[i] + vx[i ] * dt
        y[i + 1] = y[i] + vy[i ] * dt
    ...
    print("tot",tot)
    #tot=p+k
    outf = open('prj5d_vv1000_euler eng.txt', 'a')
...
plt.xlabel("year")
plt.legend(loc='upper right')
plt.title("ang momentun- sun-earth model-forward Euler ")
plt.savefig("ang momentum- sun-earth model-forward Euler.png")
plt.show()
#"""
"""
# VelocityVerlet
