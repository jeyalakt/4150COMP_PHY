...
outf = open('prj5e_jupeff10trial.txt', 'w+')
outf.write(" x\t\ty\n")
outf.close()
...
vxj[0] = 0
vyj[0] = (2.0 * (np.pi))*(5.2)
#vxj ,vyj are jupiter velocities
...
def Acc_Grav_earth(x,y):
    ...
    A_y = -G  *y* m_sun / r ** 3
    return A_x, A_y, r
def Acc_Grav_jup(xj,yj):
    rj = np.sqrt((xj) ** 2 + (yj) ** 2)
    A_xj= -G  * x*m_sun/ r ** 3
    A_yj = -G  *y* m_sun / r ** 3
    return A_xj, A_yj, rj
"""
def Acc_Grav_earth_jup1(x,xj,y,yj):
    rej = np.sqrt((x-xj) ** 2 + (y-yj) ** 2)
    A_xej = -G  * x*m_earth*m_jupiter/ r ** 3
    A_yej = -G  *y* m_earth*m_jupiter / r ** 3
    return A_xej, A_yej, rej
"""
A_x0,A_y0 ,r = Acc_Grav_earth((x[0]),(y[0]))
ax[0] =  A_x0
ay[0] =  A_y0

A_xj0,A_yj0 ,rj = Acc_Grav_jup((xj[0]),(yj[0]))
axj[0] =  A_xj0[0]
ayj[0] =  A_yj0[0]
#"""
# VelocityVerlet
for i in range(N-1):
    ...
    print("A_x",A_x)
    #A_xej, A_yej, rej = Acc_Grav_earth_jup1(x[i + 1],xj[i+1], y[i + 1],yj[i + 1])
    #ax[i+1] = A_x-A_xej
    #ay[i+1] =  A_y-A_yej
    ax[i + 1] = A_x
    ay[i + 1] = A_y
    ...
    A_xj, A_yj, rj = Acc_Grav_earth(xj[i + 1], yj[i + 1])
    print ("Axj",A_xj)
    #A_x1, A_y1, r1 = Acc_Grav_earth_jup1(xj[i + 1], yj[i + 1],x[i+1],y[i+1])
    #axj[i+1] = A_xj+A_x1
    #ayj[i+1] =  A_yj+A_y1
    axj[i + 1] = A_xj
    ayj[i + 1] = A_yj
    ...
    outf = open('prj5e_jupeff10trial.txt', 'a')
