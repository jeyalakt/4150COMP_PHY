...
filename = 'C:/Users/Bruker/4150COMP_PHY/prj5/prj5d_vv1000_vveng.txt'
...
with open(filename) as f:
    ...
    m=[float(line.split()[3]) for line in lines]
    plt.figure(1)
    plt.plot(x, 'b-', linewidth=2.0, label='potential')
    plt.plot(y, 'm-', linewidth=2.0, label='kinetic')
    plt.plot( z, 'k-', linewidth=2.0, label='angular')
    plt.plot( m, 'g-', linewidth=2.0, label='N100')
    #plt.hist(y)
"""
...
plt.title(r"  Euler -Sun Earth Model ")
plt.grid()
plt.savefig("Euler_SunEarthmodelNall.png")
plt.show()
