...
outf = open('prj4b_temp1_L2_randomspin111.txt', 'w+')
...
def main():
    ...
    outf = open('prj4b_temp1_L2_randomspin111.txt', 'a')
    outf.write(str(mcs))
    outf.write("\t\t")
    ...
    outf.write("\t\t")
    outf.write("{:.6f}".format(Cv_exact))
    outf.write("\t\t")
    outf.write("{:.6f}".format(Evariance / temp / temp))
    outf.write("\t\t")
    outf.write("{:.6f}".format(Suscept_exact))
    outf.write("\t\t")
    outf.write("{:.6f}".format(Mvariance / temp))
    outf.write("\t\t")
    outf.write("{:.6f}".format(absM_exact))
    outf.write("\t\t")
