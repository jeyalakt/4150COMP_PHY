...
def Metropolis(n_spin, lat, E, M, w):
    ...
    #print ("lat",lat)
"""
def localengy(lat, i1, j1):
    n_spin = 2
    left=lat[periodic(i1,n_spin,-1)][j1]
    right=lat[periodic(i1,n_spin,1)][j1]
    up=lat[i1][periodic(j1, n_spin, -1)]
    down=lat[i1][periodic(j1, n_spin, 1)]
    own=lat[i1][j1]
    localE=2*own*(left+right+up+down)
    print("local E",localE)
    return localE
"""
"""
def output(n_spin, mcs, temp, average):
    norm = 1 / mcs
